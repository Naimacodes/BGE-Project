...
urlPatterns = { "/FormationCreer" })
public class FormationCreer extends CrclHttpServlet {
	private static final long serialVersionUID = 1L;

	private final FormationService service = FormationService.getInstance();

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public FormationCreer() {
		super();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		request.getRequestDispatcher("/jsp/formationCreer.jsp").forward(request, response);
	}
...
//			return;
//		}
		Formation form = this.service.create();
		// 3 on sauve l'entité via le service
		this.service.save(form);
		response.sendRedirect("/crcl/FormationLister");
	}
